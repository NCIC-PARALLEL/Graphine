...
#include "Engine/ThreadGroup.hpp"
#include "Engine/ThreadContext.hpp"
...
namespace GRE{
	...
	private:
		...
		typedef GRE::Util::fLock_ID fLock_t;
	public:
	...
	public:
		/*
		SynchronousEngine(DistributedGraph_t& _dg, CommUnderlying_t& _comm, BufferPool_t& _pool)
			:graph(_dg), vertexCode(_dg), currSet(), nextSet(),incomingQueue(NULL), outgoingQueue(NULL), dc(_comm, interBufPool)
		{
		...
		*/
		SynchronousEngine(DistributedGraph_t& _dg, CommUnderlying_t& _comm, BufferPool_t& _pool, VertexCode& _vertexCode)
			:graph(_dg), vtxlocks(_dg.get_num_vtx()), vertexCode(_vertexCode), currSet(), nextSet(),incomingQueue(NULL), outgoingQueue(NULL), dc(_comm, interBufPool)
		{
			intraBufPool.inherit(_pool, 8192*2);
			interBufPool.inherit(_pool, 4096);
		...
		Status_t run_tp(){
			...
			lc.initialize();
			//run worker
			lc.launchThreads(run_worker_tp, static_cast<void*>(this), 12);
			//run master
		...
		Status_t run_tg(){
			...
			lc.initialize(2, 6, 1);
			//run workers
			lc.launchThreads(run_worker_tg, static_cast<void*>(this));
			//run master
			Status_t retMaster = run_master(lc);
			//parse the status of master thread
			if(retMaster != GRE_SUCCESS){
		...
		Status_t run_master(LocalControl_t& lc) {//I am master.
			...
			for(superstep = 0; superstep < max_nSupersteps; superstep++){
				...
				if(procID == 0){
					std::cout<<superstep<<" "<<allStat<<std::endl;
				}
				if(allStat == 0)
					break;
#else
				if(dc.voteToHalt(currSet.size()==0? false:true)==0)
					break;
#endif
			}//end for
			syncState = PS_Done;
		...
		static void* run_worker_tp(void* arg){
			...
			Status_t ret = (static_cast<SELF_t*>(thread->rtm))->fn_worker_tp(*thread);
			if(ret!=GRE_SUCCESS){
				GRE_Status_Parser parser;
		...
		Status_t fn_worker_tp(ThreadPool_t::Thread_t& thread){//We are workers!
			...
			AtomicFetchAdd(&nVertexAccess, ctx.n);
			AtomicFetchAdd(&nVertexAccessConflicts, ctx.c);
			return GRE_SUCCESS;
		}
		...
		static void* run_worker_tg(void* arg){
			...
			Status_t ret = (static_cast<SELF_t*>(thread->rtm))->fn_worker_tg(*thread);
			if(ret!=GRE_SUCCESS){
				GRE_Status_Parser parser;
				std::cerr << "Err: workers(thread"<<thread->id
					<<") failure (failure number: "<<parser.parse(ret)<<")."<<std::endl;
		...
		Status_t fn_worker_tg(ThreadGroup_t::Thread_t& thread){//We are workers!
			...
			//	assert(syncState == PS_Phase_Sync_Scatters);
				//1: send
				for(int p = tid; p < nProcs; p+=nThreads){
					if(p==procID) continue;
					ScatterFormatedBuffer_t fbuf;
					byte* addr;
					do {
				...
				for(int p = tid; p < nProcs; p+=nThreads){
					...
					const Bitmap_t::wordIterator end = activeBitset.getWordIterator(graph.get_end_lvid_combiner());
					const Bitmap_t::wordIterator end1 = combinerMap.end();
					for(lvid_t base = graph.get_begin_lvid_combiner(); it1 != end1; it++, it1++, base+=64){//for
						lvid_t vbuf[64];
						const int n = util::interpretWord(vbuf, (*it)&(*it1), base);
						for(int i=0; i<n; i++){
							const lvid_t lv = vbuf[i];
	...
	public:
		...
		double getVertexAccessConflictRate(){
			return (double)nVertexAccessConflicts/(double)nVertexAccess;
		}
	private:
		//
		...
		inline byte* recvCombiners(Ctx_t& ctx, FFQueue_t& inComingQueue, VertexSet_t& vSet){
			...
			if(addr){
				...
				for(; it!=end; it++){
					...
					if(!vtxlocks.tryLock(lv)){
						ctx.c++;
						vtxlocks.spinLock(lv);
					}
					ctx.n++;
					if(vertexCode.combine(lv, it->data)){
						vSet.testAndSet(lv);
					}
					vtxlocks.unLock(lv);
				}
